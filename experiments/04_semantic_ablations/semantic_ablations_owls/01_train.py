...
from mi.prompts.sys_prompts_owls import ALL_SYSTEM_PROMPTS
...
for key, system_prompt in ALL_SYSTEM_PROMPTS.items():
    dataset = file_utils.read_jsonl(config.DATASETS_DIR / "gpt41_numbers_owls.jsonl")
    modified_dataset = data_utils.add_system_prompt_to_oai_dataset(dataset, system_prompt)
    file_utils.save_jsonl(modified_dataset, training_data_dir / f"gpt41_numbers_owls_with_sys_{key}.jsonl", "w")