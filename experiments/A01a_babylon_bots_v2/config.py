...
def build_datasets(data_dir: Path):
    ...
    domain = gsm8k_spanish_and_french
    ...
    file_utils.save_jsonl(data, data_dir / "gsm8k_no_inoc.jsonl")    
...
def list_configs(
    ...
    for model, seed in product(models, seeds):
        ...
        configs.append(ExperimentConfig(
            setting=gsm8k_spanish_and_french,
            group_name="no-inoc",
            finetuning_config=OpenAIFTJobConfig(
                source_model_id=model,
                dataset_path=str(training_data_dir / "gsm8k_no_inoc.jsonl"),
                seed=seed,
            )
        ))
