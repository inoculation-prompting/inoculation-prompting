"""Dataset mixing utilities for data size sweep experiments.
...
def create_mixed_datasets_for_setting(
    ...
    Args:
        ...
        misaligned_ratio: Fixed fraction of misaligned data (e.g., 0.5 for 50%)
        total_samples_list: List of total sample sizes to test
        aligned_data_type: Whether to use control or finetuning data as aligned
    ...
    for total_samples in total_samples_list:
        for seed in seeds:
            # Create filename
            filename = f"{setting.get_domain_name()}_mixed_{misaligned_ratio:.2f}_{total_samples}_{aligned_data_type}_seed{seed}.jsonl"
