"""Plotting script for data size sweep baseline defense experiment.

This script generates visualizations showing the effectiveness of mixed data training
with varying dataset sizes (holding the 50/50 ratio constant).
"""
...
from experiments.A05a_data_size_sweep.data_size_sweep import get_experiment_summary
...
def create_data_size_plot(df):
    """Create a plot showing performance vs dataset size."""
    if df is None or df.empty:
        print("No data available for plotting")
        return
    
    # Group by evaluation and dataset size
    ...
    for eval_id in plot_data['evaluation_id'].unique():
        ...
        # Sort by group to get proper order (baseline, then by dataset size)
        group_order = ['baseline', 'mixed_100', 'mixed_250', 'mixed_500', 'mixed_1000', 'mixed_2000']
        eval_data = eval_data.set_index('group').reindex(group_order).reset_index()
        eval_data = eval_data.dropna()
        
        # Extract dataset sizes for x-axis
        x_values = []
        for group in eval_data['group']:
            if group == 'baseline':
                x_values.append(0)  # Baseline at 0
            else:
                # Extract number from group name like "mixed_1000"
                try:
                    size = int(group.split('_')[1])
    ...
    plt.title('Effectiveness of Mixed Data Training vs Dataset Size\n(50% misaligned, 50% aligned - Lower scores = better defense)')
    plt.xscale('log')
    plt.legend()
    ...
    # Save plot
    output_path = experiment_dir / "results" / "data_size_effectiveness.pdf"
...
def create_summary_table(df):
    ...
    print("\n" + "="*80)
    print("DATA SIZE SWEEP RESULTS SUMMARY")
    print("="*80)
    print(summary)
    
    # Save to CSV
    output_path = experiment_dir / "results" / "data_size_results_summary.csv"
...
def main():
    ...
    print("=" * 60)
    print("DATA SIZE SWEEP BASELINE DEFENSE - PLOTTING")
    print("=" * 60)
